...
// Macro class
var Macro = function (name, index, length, prefix) {
    this.name = name;
    this.start = index;
    this.length = length;
    this.prefix = prefix;
...
MacroContext.prototype.resolveMacros = function (content, macros) {
    ...
    content = content.replace(regex, function (match) {
        ...
        //return (macro.prefix?macro.prefix:'') + "' + " +  macros[macro.name].apply(null, macro.getArguments()) + " + '";
    });
...
var specs = {
    outside: {
        "^@(\\w+)\\(|([^\\\\])@(\\w+)\\(": function (match, name, prefix, _name, index, length) {
            ...
            var macro = new Macro(name, prefix ? index + 1 : index, length, prefix);
